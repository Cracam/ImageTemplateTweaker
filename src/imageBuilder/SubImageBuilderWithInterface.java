...
import static staticFunctions.StaticImageEditing.createImageView;
 ...
 */
public class SubImageBuilderWithInterface extends ImageBuilder {

        private SubImageBuilderInterface subInterface;
        static float QUALITY_FACTOR = 2;
        private final BooleanProperty changed = new SimpleBooleanProperty(false);
        private final Node loaderNode;
    private ImageView preview;
         ...
         * @param upperLayer
         */
        public SubImageBuilderWithInterface(DesignBuilder batcher, Node loaderNode, float size_x, float size_y, Layer upperLayer) {
                String nameC = ((Element) loaderNode).getAttribute("name");
                super(batcher, nameC, size_x * QUALITY_FACTOR, size_y * QUALITY_FACTOR);
        ...
        public void refresh() {
                DRYrefresh();
                setChanged(true);
             //   subInterface.refreshPreview( createImageView(this.getImageOut()));
        }
                ...
                for (int i = 0; i < nodeLayerList.getLength(); i++) {
                        if (nodeLayerList.item(i).getNodeType() == Node.ELEMENT_NODE) { //To avoid text node and comment node
                                try {
                                        ...
                                        Logger.getLogger(SubImageBuilderWithInterface.class.getName()).log(Level.SEVERE, null, ex);
                                }
                        }
                }
        }
